...
const translations = {
    en: {
        ...
        'settings.apiKeyLabel': 'API Keys',
        'settings.keyPlaceholder': 'Enter your API key...',
        'settings.keyNote': 'Your keys are stored locally and sent only to their respective providers.',
        'settings.invalidKey': 'Please enter a valid API key.',

        // Auth Modal
        'auth.title': 'API Keys Required',
        'auth.subtitle': 'Add an API key for at least one provider to start generating.',
        'auth.keyLabel': 'API Key',
        'auth.keyPlaceholder': 'Paste your access key value...',
        'auth.keyNote': 'Enter the API key from your provider dashboard.',
        'auth.initBtn': 'Initialize Studio',
        'auth.createKey': 'Open Settings to add your provider API keys →',
    ...
    zh: {
        ...
        'settings.keyNote': '您的密钥仅存储在本地，并仅发送到各自提供商。',
        'settings.invalidKey': '请输入有效的 API 密钥。',

        // Auth Modal
        'auth.title': '需要 API 密钥',
        'auth.subtitle': '添加至少一个提供商的 API 密钥以开始生成。',
        'auth.keyLabel': 'API 密钥',
        'auth.keyPlaceholder': '输入您的 API 密钥...',
        'auth.keyNote': '请输入您提供商仪表板中的 API 密钥。',
        'auth.initBtn': '初始化工作室',
        'auth.createKey': '打开设置添加您的提供商 API 密钥 →',
